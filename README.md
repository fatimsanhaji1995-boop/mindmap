...
Add these in Vercel project settings (or local `.env`, see `.env.example`):
...
- `VITE_API_BASE_URL` *(optional)* - base URL for API when frontend and API are not served from the same origin (example: `http://localhost:3000`).
...
# For local API routes (/api/*), run via Vercel CLI:
vercel dev
```
