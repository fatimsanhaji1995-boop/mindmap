...
function App() {
  ...
  const cloudApiEnabled = !import.meta.env.DEV || Boolean(apiBaseUrl);

  const buildApiUrl = useCallback((path) => `${apiBaseUrl}${path}`, [apiBaseUrl]);
  ...
  const handleAuth = async (mode) => {
    if (!cloudApiEnabled) {
      alert('Cloud API is disabled in local Vite dev. Set VITE_API_BASE_URL or use vercel dev.');
      return;
    }
    ...
    const payload = await parseApiResponse(response);
    if (!response.ok) {
      alert(payload?.error || `Failed to ${mode}. Check API deployment/config.`);
      return;
    }
  ...
  const saveGraphToCloud = async () => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/api/graphs/${encodeURIComponent(graphId.trim())}`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const loadGraphFromCloud = async () => {
    ...
    try {
      ...
      if (!payload?.graph?.data) {
        alert('Graph payload is invalid.');
        return;
      }
        ...
        >
          <div className="space-y-4">
            <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Uses <code>/api/*</code> endpoints. In local Vite dev, set <code>VITE_API_BASE_URL</code> (or use <code>vercel dev</code>) to enable cloud actions.</p>
                <Input
                  type="email"
